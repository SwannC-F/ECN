    ...
    <>
      <div className="w-full relative mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {files.fr && (
            <div className="glass-card bg-zinc-900/60 p-5 rounded-xl border border-zinc-800/80 group hover:border-emerald-500/30 transition-all flex flex-col justify-between h-full relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-6">
                ...
                <div>
                  <h4 className="font-semibold text-zinc-200">Version Française</h4>
                  <p className="text-xs text-zinc-500 font-mono">Format PDF</p>
                </div>
              </div>
                ...
                <a 
                  ...
                  className={cn(
                    ...
                    downloadedStatus === 'fr' 
                      ...
                      : "bg-zinc-950 text-zinc-300 border-zinc-800 hover:bg-zinc-900 hover:text-white hover:border-zinc-700"
                  )}
                >
          ...
          {files.en && (
            <div className="glass-card bg-zinc-900/60 p-5 rounded-xl border border-zinc-800/80 group hover:border-emerald-500/30 transition-all flex flex-col justify-between h-full relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-6">
                ...
                <div>
                  <h4 className="font-semibold text-zinc-200">English Version</h4>
                  <p className="text-xs text-zinc-500 font-mono">PDF Format</p>
                </div>
              </div>
        ...
        {/* Device hints */}
        {isMobile && (
           <p className="text-[10px] text-zinc-600 mt-3 flex items-center gap-1.5"><Smartphone className="w-3 h-3" /> Optimisé pour une lecture mobile via le visualiseur natif de votre appareil.</p>
        )}
        {!isMobile && (
           <p className="text-[10px] text-zinc-600 mt-3 flex items-center gap-1.5"><Monitor className="w-3 h-3" /> Lecture grand format incrustée au sein de notre interface Liseuse disponible.</p>
        )}
      </div>
      ...
      <AnimatePresence>
        {activePdfUrl && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[110] flex items-center justify-center bg-zinc-950/95 backdrop-blur-xl p-4 md:p-12"
          >
            <div className="w-full h-full max-w-6xl max-h-[90vh] bg-zinc-900 rounded-2xl border border-zinc-800 shadow-2xl flex flex-col overflow-hidden relative">
              {/* Toolbar */}
              <div className="h-16 border-b border-zinc-800 bg-zinc-950 flex items-center justify-between px-6 shrink-0 z-20">
                <div className="flex items-center gap-3">
                  <Diamond className="w-5 h-5 text-emerald-500" />
                  <span className="text-sm font-serif italic text-zinc-200">ECN Intelligence Reader</span>
                </div>
                <div className="flex items-center gap-4">
                  <a 
                    href={activePdfUrl} 
                    download 
                    className="hidden sm:flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 text-sm text-zinc-400 hover:text-red-400 transition-colors p-2 hover:bg-red-400/10 rounded-md"
                  >
              ...
              <div className="flex-1 w-full bg-zinc-800 relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-900 text-zinc-500 z-0 gap-4">
                  <div className="animate-pulse flex items-center justify-center p-4 bg-zinc-800 rounded-full">
